...
public class FontRenderer
{
    ...
    public void func_27278_a(String s, int i, int j, int k, int l)
    {
        ...
        do
        {
            ...
            String s1;
            for(s1 = (new StringBuilder()).append(as[i1++]).append(" ").toString(); i1 < as.length && getStringWidth((new StringBuilder()).append(s1).append(as[i1]).toString()) < k; s1 = (new StringBuilder()).append(s1).append(as[i1++]).append(" ").toString()) { }
            int j1;
            for(; getStringWidth(s1) > k; s1 = s1.substring(j1))
            {
                for(j1 = 0; getStringWidth(s1.substring(0, j1 + 1)) <= k; j1++) { }
                if(s1.substring(0, j1).trim().length() > 0)
                {
                    drawString(s1.substring(0, j1), i, j, l);
                    j += 8;
                }
            }
    ...
    public int func_27277_a(String s, int i)
    {
        ...
        do
        {
            ...
            String s1;
            for(s1 = (new StringBuilder()).append(as[j++]).append(" ").toString(); j < as.length && getStringWidth((new StringBuilder()).append(s1).append(as[j]).toString()) < i; s1 = (new StringBuilder()).append(s1).append(as[j++]).append(" ").toString()) { }
            int l;
            for(; getStringWidth(s1) > i; s1 = s1.substring(l))
            {
                for(l = 0; getStringWidth(s1.substring(0, l + 1)) <= i; l++) { }
                if(s1.substring(0, l).trim().length() > 0)
                {
                    k += 8;
                }
            }
