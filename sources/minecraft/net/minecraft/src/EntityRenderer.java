...
public class EntityRenderer
{
    ...
    private void orientCamera(float f)
    {
        ...
        field_27288_B = mc.renderGlobal.func_27307_a(d, d1, d2, f);
    }
    ...
    private void addRainParticles()
    {
        ...
        random.setSeed((long)field_1386_j * 0x12a7ce5fL);
        EntityLiving entityliving = mc.renderViewEntity;
        ...
        for(int i1 = 0; i1 < (int)(100F * f * f); i1++)
        {
            ...
            if(l1 > j + byte0 || l1 < j - byte0 || !world.getWorldChunkManager().func_4073_a(j1, k1).func_27077_d())
            {
                continue;
            }
        ...
        if(l > 0 && random.nextInt(3) < field_27287_H++)
        {
            field_27287_H = 0;
            if(d1 > entityliving.posY + 2D && !world.canBlockSeeTheSky(MathHelper.floor_double(entityliving.posX), MathHelper.floor_double(entityliving.posY), MathHelper.floor_double(entityliving.posZ)))
            {
    ...
    protected void func_27286_d(float f)
    {
        ...
        BiomeGenBase abiomegenbase[] = world.getWorldChunkManager().func_4069_a(i - l, k - l, l * 2 + 1, l * 2 + 1);
        int i1 = 0;
        for(int j1 = i - l; j1 <= i + l; j1++)
        {
            for(int l1 = k - l; l1 <= k + l; l1++)
            {
                ...
                int j2 = world.findTopSolidBlock(j1, l1);
                if(j2 < 0)
                {
                    j2 = 0;
                }
                ...
                if(l2 != j3)
                {
                    random.setSeed(j1 * j1 * 3121 /*GL_RGBA_MODE*/ + j1 * 0x2b24abb + l1 * l1 * 0x66397 + l1 * 13761);
                    float f4 = (float)field_1386_j + f;
                    float f6 = ((float)(field_1386_j & 0x1ff) + f) / 512F;
                    float f7 = random.nextFloat() + f4 * 0.01F * (float)random.nextGaussian();
                    float f8 = random.nextFloat() + f4 * (float)random.nextGaussian() * 0.001F;
                    double d5 = (double)((float)j1 + 0.5F) - entityliving.posX;
                    double d6 = (double)((float)l1 + 0.5F) - entityliving.posZ;
                    float f11 = MathHelper.sqrt_double(d5 * d5 + d6 * d6) / (float)l;
                    tessellator.startDrawingQuads();
                    float f12 = world.getLightBrightness(j1, 128, l1);
                    GL11.glColor4f(f12, f12, f12, ((1.0F - f11 * f11) * 0.3F + 0.5F) * f1);
                    tessellator.setTranslationD(-d * 1.0D, -d1 * 1.0D, -d2 * 1.0D);
                    tessellator.addVertexWithUV(j1 + 0, l2, (double)l1 + 0.5D, 0.0F * f2 + f7, ((float)l2 * f2) / 4F + f6 * f2 + f8);
                    tessellator.addVertexWithUV(j1 + 1, l2, (double)l1 + 0.5D, 1.0F * f2 + f7, ((float)l2 * f2) / 4F + f6 * f2 + f8);
                    tessellator.addVertexWithUV(j1 + 1, j3, (double)l1 + 0.5D, 1.0F * f2 + f7, ((float)j3 * f2) / 4F + f6 * f2 + f8);
                    tessellator.addVertexWithUV(j1 + 0, j3, (double)l1 + 0.5D, 0.0F * f2 + f7, ((float)j3 * f2) / 4F + f6 * f2 + f8);
                    tessellator.addVertexWithUV((double)j1 + 0.5D, l2, l1 + 0, 0.0F * f2 + f7, ((float)l2 * f2) / 4F + f6 * f2 + f8);
                    tessellator.addVertexWithUV((double)j1 + 0.5D, l2, l1 + 1, 1.0F * f2 + f7, ((float)l2 * f2) / 4F + f6 * f2 + f8);
                    tessellator.addVertexWithUV((double)j1 + 0.5D, j3, l1 + 1, 1.0F * f2 + f7, ((float)j3 * f2) / 4F + f6 * f2 + f8);
                    tessellator.addVertexWithUV((double)j1 + 0.5D, j3, l1 + 0, 0.0F * f2 + f7, ((float)j3 * f2) / 4F + f6 * f2 + f8);
        ...
        {
            for(int i2 = k - l; i2 <= k + l; i2++)
            {
                BiomeGenBase biomegenbase1 = abiomegenbase[i1++];
                if(!biomegenbase1.func_27077_d())
                {
                    continue;
                }
                ...
                if(i3 != k3)
                {
                    float f5 = ((float)(field_1386_j + k1 * k1 * 3121 /*GL_RGBA_MODE*/ + k1 * 0x2b24abb + i2 * i2 * 0x66397 + i2 * 13761 & 0x1f) + f) / 32F;
                    double d3 = (double)((float)k1 + 0.5F) - entityliving.posX;
                    double d4 = (double)((float)i2 + 0.5F) - entityliving.posZ;
                    float f9 = MathHelper.sqrt_double(d3 * d3 + d4 * d4) / (float)l;
                    tessellator.startDrawingQuads();
                    float f10 = world.getLightBrightness(k1, 128, i2) * 0.85F + 0.15F;
                    GL11.glColor4f(f10, f10, f10, ((1.0F - f9 * f9) * 0.5F + 0.5F) * f1);
                    tessellator.setTranslationD(-d * 1.0D, -d1 * 1.0D, -d2 * 1.0D);
                    tessellator.addVertexWithUV(k1 + 0, i3, (double)i2 + 0.5D, 0.0F * f3, ((float)i3 * f3) / 4F + f5 * f3);
                    tessellator.addVertexWithUV(k1 + 1, i3, (double)i2 + 0.5D, 1.0F * f3, ((float)i3 * f3) / 4F + f5 * f3);
                    tessellator.addVertexWithUV(k1 + 1, k3, (double)i2 + 0.5D, 1.0F * f3, ((float)k3 * f3) / 4F + f5 * f3);
                    tessellator.addVertexWithUV(k1 + 0, k3, (double)i2 + 0.5D, 0.0F * f3, ((float)k3 * f3) / 4F + f5 * f3);
                    tessellator.addVertexWithUV((double)k1 + 0.5D, i3, i2 + 0, 0.0F * f3, ((float)i3 * f3) / 4F + f5 * f3);
                    tessellator.addVertexWithUV((double)k1 + 0.5D, i3, i2 + 1, 1.0F * f3, ((float)i3 * f3) / 4F + f5 * f3);
                    tessellator.addVertexWithUV((double)k1 + 0.5D, k3, i2 + 1, 1.0F * f3, ((float)k3 * f3) / 4F + f5 * f3);
                    tessellator.addVertexWithUV((double)k1 + 0.5D, k3, i2 + 0, 0.0F * f3, ((float)k3 * f3) / 4F + f5 * f3);
                    tessellator.setTranslationD(0.0D, 0.0D, 0.0D);
                    tessellator.draw();
                }
            }
    ...
    private void updateFogColor(float f)
    {
        ...
        float f10 = field_1382_n + (field_1381_o - field_1382_n) * f;
        fogColorRed *= f10;
        fogColorGreen *= f10;
        fogColorBlue *= f10;
        if(mc.gameSettings.anaglyph)
