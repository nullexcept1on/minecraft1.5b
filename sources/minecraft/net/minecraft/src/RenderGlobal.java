    ...
    implements IWorldAccess
{
    ...
    public void renderEntities(Vec3D vec3d, ICamera icamera, float f)
    {
        ...
        for(int i = 0; i < worldObj.field_27173_e.size(); i++)
        {
            Entity entity = (Entity)worldObj.field_27173_e.get(i);
            field_1422_K++;
        ...
        for(int j = 0; j < list.size(); j++)
        {
            ...
            if(entity1.isInRangeToRenderVec3D(vec3d) && icamera.isBoundingBoxInFrustum(entity1.boundingBox) && (entity1 != mc.renderViewEntity || mc.gameSettings.thirdPersonView || mc.renderViewEntity.isPlayerSleeping()) && worldObj.blockExists(MathHelper.floor_double(entity1.posX), MathHelper.floor_double(entity1.posY), MathHelper.floor_double(entity1.posZ)))
            {
                field_1422_K++;
    ...
    public int sortAndRender(EntityLiving entityliving, int i, double d)
    {
        ...
        int k = 0;
        if(occlusionEnabled && mc.gameSettings.field_27342_h && !mc.gameSettings.anaglyph && i == 0)
        {
            int l = 0;
            int i1 = 16;
            func_962_a(l, i1);
    ...
    public void renderSky(float f)
    {
        ...
        if(af != null)
        {
            ...
            for(int j = 0; j <= i; j++)
            {
                ...
                float f16 = MathHelper.cos(f12);
                tessellator.addVertex(f14 * 120F, f16 * 120F, -f16 * 40F * af[3]);
            }
        ...
        tessellator.startDrawingQuads();
        tessellator.addVertexWithUV(-f13, 100D, -f13, 0.0D, 0.0D);
        tessellator.addVertexWithUV(f13, 100D, -f13, 1.0D, 0.0D);
        tessellator.addVertexWithUV(f13, 100D, f13, 1.0D, 1.0D);
        tessellator.addVertexWithUV(-f13, 100D, f13, 0.0D, 1.0D);
        tessellator.draw();
        f13 = 20F;
        ...
        tessellator.draw();
        GL11.glDisable(3553 /*GL_TEXTURE_2D*/);
        float f15 = worldObj.getStarBrightness(f) * f6;
        if(f15 > 0.0F)
        {
    ...
    public void clipRenderersByFrustrum(ICamera icamera, float f)
    {
        for(int i = 0; i < worldRenderers.length; i++)
        {
            if(!worldRenderers[i].canRender() && (!worldRenderers[i].isInFrustum || (i + frustrumCheckOffset & 0xf) == 0))
            {
