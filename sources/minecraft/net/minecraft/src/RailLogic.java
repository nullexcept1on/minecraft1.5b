...
class RailLogic
{
    ...
    public RailLogic(BlockRail blockrail, World world, int i, int j, int k)
    {
        ...
        if(BlockRail.func_27039_a((BlockRail)Block.blocksList[l]))
        {
            field_27212_f = true;
            i1 &= -9;
        } else
    ...
    {
        if(BlockRail.func_27040_h(worldObj, i, j, k))
        {
            return true;
        }
        if(BlockRail.func_27040_h(worldObj, i, j + 1, k))
        {
            return true;
        }
        return BlockRail.func_27040_h(worldObj, i, j - 1, k);
    }

    private RailLogic getMinecartTrackLogic(ChunkPosition chunkposition)
    {
        if(BlockRail.func_27040_h(worldObj, chunkposition.x, chunkposition.y, chunkposition.z))
        {
            return new RailLogic(minecartTrack, worldObj, chunkposition.x, chunkposition.y, chunkposition.z);
        }
        if(BlockRail.func_27040_h(worldObj, chunkposition.x, chunkposition.y + 1, chunkposition.z))
        {
    ...
    private void func_788_d(RailLogic raillogic)
    {
        ...
        if(byte0 == 0)
        {
            if(BlockRail.func_27040_h(worldObj, trackX, trackY + 1, trackZ - 1))
            {
                byte0 = 4;
            }
            if(BlockRail.func_27040_h(worldObj, trackX, trackY + 1, trackZ + 1))
        ...
        if(byte0 == 1)
        {
            if(BlockRail.func_27040_h(worldObj, trackX + 1, trackY + 1, trackZ))
            {
                byte0 = 2;
            }
            if(BlockRail.func_27040_h(worldObj, trackX - 1, trackY + 1, trackZ))
